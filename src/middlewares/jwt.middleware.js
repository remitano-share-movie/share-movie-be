...
const middleware = async (req, res, next)=>{
    try {
        ...
        if (!user)
            return res.status(UNAUTHORIZED).send({ message: 'Access token do not match', errorCode: 2 })
        ...
        return res.status(UNAUTHORIZED).send({ message: 'Access token invalid', errorCode: 4 })
    }
}
